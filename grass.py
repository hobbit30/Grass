...
def colorful_log(proxy, device_id, message_type, message_content, is_sent=False, mode=None):
    ...
    if "Working..." in message_type or "503" in message_content:
        return 
                ...
                while True:
                    if is_authenticated and not has_received_action:
                        colorful_log(
                            ...
                            message_content="Waiting for HTTP_REQUEST",
                            mode=mode
                        )
...
async def main():
    ...
    print(f"{Fore.CYAN}As@30 | Grass Farming{Style.RESET_ALL}")

    
    with open('user_id.txt', 'r') as id_file:
    ...
    tasks = [asyncio.ensure_future(connect_to_wss(i, _user_id, "extension")) for i in local_proxies]
